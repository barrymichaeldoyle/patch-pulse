...
export const slackOAuthCallback = httpAction(async (ctx, request) => {
  ...
  try {
    ...
    await ctx.runMutation(internal.subscribers.upsertSlackWorkspace, {
      ...
      webhookConfigurationUrl: data.incoming_webhook.configuration_url,
    });

    await fetch(data.incoming_webhook.url, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ text: "*PatchPulse* has been connected successfully! 🎉" }),
    });
