    ...
    blocks: [
      {
        ...
        hint: { type: "plain_text", text: "Space-separate multiple packages: react vue typescript" },
        element: {
      ...
      },
      {
        ...
        hint: { type: "plain_text", text: "Leave blank to receive notifications via DM" },
        optional: true,
        element: {
      ...
      },
      {
        ...
        element: {
          ...
          initial_option: { text: { type: "plain_text", text: "All updates (patch+)" }, value: "patch" },
          options: [
            { text: { type: "plain_text", text: "All updates (patch+)" }, value: "patch" },
            { text: { type: "plain_text", text: "Minor & major only" }, value: "minor" },
...
export function moveChannelModalView(subscriptionId: string, packageName: string) {
  return {
    type: "modal",
    ...
    blocks: [
      {
        type: "section",
        text: { type: "mrkdwn", text: `Move *${packageName}* notifications to a different channel.` },
      },
      {
        type: "input",
        block_id: "channel_block",
...
export async function openMoveChannelModal(
  ...
  const response = await fetch("https://slack.com/api/views.open", {
    ...
    body: JSON.stringify({ trigger_id: triggerId, view: moveChannelModalView(subscriptionId, packageName) }),
  });
  const data = await response.json();
  if (!data.ok) throw new Error(`Slack views.open error: ${data.error}`);
}

export async function openTrackModal(token: string, triggerId: string): Promise<void> {
  const response = await fetch("https://slack.com/api/views.open", {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
...
export async function publishAppHome(
  ...
  if (entries && entries.length > 0) {
    ...
    const groups = new Map<string, { heading: string; entries: HomePackageEntry[] }>();
    for (const entry of [...entries].sort((a, b) => a.packageName.localeCompare(b.packageName))) {
      const key = entry.channelId ? `channel:${entry.channelId}` : `dm:${entry.userId}`;
    ...
    for (const [, { heading, entries: groupEntries }] of sortedGroups) {
      ...
      for (const entry of groupEntries) {
        ...
        const allThresholds: Array<{ label: string; value: "patch" | "minor" | "major" }> = [
    ...
    // Last checked footer
    const lastChecked = entries.reduce((max, e) => Math.max(max, e.lastChecked ?? 0), 0);
    if (lastChecked > 0) {
      blocks.push({
        type: "context",
        elements: [{ type: "mrkdwn", text: `Last checked ${relativeTime(lastChecked)}` }],
      });
    }
  } else {
    blocks.push({
  ...
  blocks.push(
    ...
    {
      type: "context",
      elements: [{ type: "mrkdwn", text: "*/npmtrack <pkg> #channel* — track in a channel" }],
    },
    {
      type: "context",
      elements: [{ type: "mrkdwn", text: "*/npmtrack <pkg> minor|major* — set update threshold" }],
    },
    {
      type: "context",
      elements: [{ type: "mrkdwn", text: "*/npmuntrack <pkg>* — stop tracking" }],
    },
    {
      type: "context",
      elements: [{ type: "mrkdwn", text: "*/npmlist* — list all tracked packages   •   */npmhelp* — full reference" }],
    },
    {
      type: "context",
      elements: [
  ...
  const response = await fetch("https://slack.com/api/views.publish", {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
    body: JSON.stringify({ user_id: viewingUserId, view: { type: "home", blocks } }),
  });
  const data = await response.json();
