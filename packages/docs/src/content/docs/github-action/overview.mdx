...
- **Auto-merge** — optionally auto-merge patch (or patch + minor) PRs once CI passes
- **No stale PR pile-up** — one branch per group (`patch-pulse/<name>`), re-used on each run; the PR body updates in place if the branch already has an open PR

## How it works

1. The action runs `npx patch-pulse --json --hide-clean` in your repository
2. Outdated packages are collected and grouped according to your configuration
...
5. The branch is pushed and a pull request is opened (or the existing one is updated)
...
| Ref              | Behaviour                                                                                                                                               |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `@action/v0`     | **Recommended.** Moving tag — always the latest `v1.x` release. You get new features and fixes automatically, but breaking changes (a `v2`) are opt-in. |
| `@action/v0.2.3` | Exact pin — nothing changes until you update the ref yourself. Use this in environments that require full reproducibility.                              |
| `@main`          | Unreleased HEAD — avoid in production. Useful only for testing unreleased changes.                                                                      |

A `v2` will only be cut if a change would break existing workflow configurations. Additive changes (new inputs, new behaviour behind a flag) stay in `v1`.
