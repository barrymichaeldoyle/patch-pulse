...
export function getDependencySections(packageJson) {
  const sections = {};
...
export function getAllDependencyNames(packageJson) {
  return Object.keys(
...
export function getDependencyVersion(packageJson, packageName) {
  for (const field of PACKAGE_JSON_DEPENDENCY_FIELDS) {
...
export function preserveWildcardPrefix(currentVersion, latestVersion) {
...
export function hasWildcardPrefix(version) {
  return /^[\^~>=<]+/.test(version);
}
...
export async function fetchNpmLatestVersionCached(packageName, options = {}) {
...
export async function prefetchNpmPackageVersions(packageNames, options = {}) {
  const {
    cache,
    concurrency = 10,
    createMeta,
...
export async function checkNpmDependencyStatuses(dependencies, options = {}) {
  const {
    cache,
    category,
    concurrency = 10,
...
export class PackageVersionCache {
  #cache = new Map();
  #defaultTtlMs;
  #ttlByPackageName;
...
export function getNpmLatestVersion(manifest) {
  return manifest?.['dist-tags']?.latest;
}

export async function fetchNpmPackageManifest(packageName, options = {}) {
